...
frames="${1:-${ACRYL_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/acryl-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	acryl_screen "Installing ACRYL" "Downloading ACRYL$(acryl_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
