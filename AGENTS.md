...
## Testing ACRYL Interactive Mode with tmux

To test ACRYL's TUI in a controlled terminal environment:

```bash
# Create tmux session with specific dimensions
tmux new-session -d -s acryl-test -x 80 -y 24

# Start ACRYL from source
tmux send-keys -t acryl-test "cd /Users/kevin/pi/acryl && ./acryl.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t acryl-test -p

# Send input
tmux send-keys -t acryl-test "your prompt here" Enter

# Send special keys
tmux send-keys -t acryl-test Escape
tmux send-keys -t acryl-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t acryl-test
```
...
- **Internal changes (from issues)**: `Fixed foo bar ([#123](https://github.com/PrimeIntellect-ai/acryl/issues/123))`
- **External contributions**: `Added feature X ([#456](https://github.com/PrimeIntellect-ai/acryl/pull/456) by [@username](https://github.com/username))`
