...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		delete process.env.ACRYL_KERNEL_PYTHON;
		delete process.env.ACRYL_KERNEL_VENV;
	...
	it("returns the configured kernel venv directory", () => {
		...
		process.env.ACRYL_KERNEL_VENV = venv;
...
`,
	...
	it("rejects ACRYL_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects ACRYL_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects ACRYL_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid ACRYL_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.ACRYL_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/ACRYL_KERNEL_PYTHON points to a Python missing/);
	});
});
