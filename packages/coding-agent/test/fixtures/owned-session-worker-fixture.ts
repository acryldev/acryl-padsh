...
if (process.env.ACRYL_TEST_STDIN_TTY) {
	Object.defineProperty(process.stdin, "isTTY", { value: process.env.ACRYL_TEST_STDIN_TTY === "1" });
}
const pidPath = process.env.ACRYL_TEST_OWNED_PID_PATH;
installOwnedSessionWorkerOwnerWatch();

if (process.env.ACRYL_INTERNAL_OWNED_WORKER === "1") {
	if (pidPath) {
		writeFileSync(`${pidPath}.ppid`, `${process.ppid}\n`);
		writeFileSync(`${pidPath}.profile`, `${process.env.ACRYL_INTERNAL_OWNED_PROFILE ?? ""}\n`);
	...
	if (process.env.ACRYL_TEST_KEEP_ALIVE === "1") {
		setInterval(() => {}, 1000);
	}
	if (args.includes("--mode") && args.includes("rpc")) {
		...
		attachJsonlLineReader(process.stdin, (line) => {
			...
			if (process.env.ACRYL_TEST_EXIT_ZERO_ON_COMMAND === command.type) {
				process.exit(0);
			}
			if (process.env.ACRYL_TEST_CRASH_ON_COMMAND === command.type) {
				process.exit(1);
			}
			if (command.type === "ack_result") {
				if (process.env.ACRYL_TEST_CRASH_ON_ACK === "1" && pidPath && !existsSync(`${pidPath}.crashed`)) {
					writeFileSync(`${pidPath}.crashed`, "crashed\n");
					const recoveryPath = process.env.ACRYL_INTERNAL_OWNED_RECOVERY_DESCRIPTOR;
					if (recoveryPath) {
						writeFileSync(
							recoveryPath,
							`${JSON.stringify({
			...
			if (process.env.ACRYL_TEST_INVALID_RPC_OUTPUT === "1") {
				process.stdout.write("truncated-json\n");
				process.stdout.write("null\n");
			}
			if (process.env.ACRYL_TEST_REVERSE_RPC_RESPONSES === "1") {
