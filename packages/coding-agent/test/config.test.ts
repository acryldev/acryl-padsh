...
describe("detectInstallMethod", () => {
	...
	test("leaves Homebrew installs under Homebrew ownership", () => {
		...
		expect(getSelfUpdateUnavailableInstruction("prime-agent")).toBe("Update with: brew upgrade acryl");
		expect(getUpdateInstruction("prime-agent")).toBe("Update with: brew upgrade acryl");
	});
...
describe("session paths", () => {
	test("uses the short app-prefixed session dir env var", () => {
		expect(ENV_SESSION_DIR).toBe("ACRYL_SESSION_DIR");
	});
