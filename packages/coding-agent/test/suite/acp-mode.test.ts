...
function acpUpdatePhase(update: Record<string, unknown>): unknown {
	...
	const primeMetadata = metadata?.[ACRYL_META_NAMESPACE] as Record<string, unknown> | undefined;
...
describe("ACP mode end to end", () => {
	...
	it("emits score-safe quiescence metadata with outstanding work and budget", async () => {
		...
		await vi.waitFor(() =>
			expect(
				updates.some((update) => update.update?._meta?.[ACRYL_META_NAMESPACE]?.phase === "terminalQuiescence"),
			).toBe(true),
		);
		const correlated = updates
			.map((update) => update.update?._meta?.[ACRYL_META_NAMESPACE])
	...
	it("treats unused autonomous capacity as terminal lifecycle telemetry", async () => {
		...
		await vi.waitFor(() =>
			expect(
				updates.some((item) => item.update?._meta?.[ACRYL_META_NAMESPACE]?.phase === "terminalQuiescence"),
			).toBe(true),
		);
		const meta = updates.map((item) => item.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
	...
	it("reports a descendant-driven parent failure in the terminal outcome", async () => {
		...
		await vi.waitFor(() =>
			expect(
				updates
					.map((item) => item.update?._meta?.[ACRYL_META_NAMESPACE])
	...
	it("finalizes a child-backed turn only after the strong RLM barrier", async () => {
		...
		let metadata = updates.map((item) => item.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
		...
		releaseBarrier();
		await vi.waitFor(() => {
			metadata = updates.map((item) => item.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
	...
	it("coalesces duplicate cancellation notifications under one stop owner", async () => {
		...
		await vi.waitFor(() =>
			expect(updates.some((item) => item.update?._meta?.[ACRYL_META_NAMESPACE]?.phase === "responseBoundary")).toBe(
				true,
			),
		);
	...
	it("propagates a failed roster read at quiescence emission", async () => {
		...
		const metadata = updates.map((u) => u.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
	...
	it("keeps global sequences and causal turn ids across sequential prompts", async () => {
		...
		await vi.waitFor(() =>
			expect(
				updates.filter((u) => u.update?._meta?.[ACRYL_META_NAMESPACE]?.phase === "terminalQuiescence"),
			).toHaveLength(2),
		);
		const metadata = updates.map((u) => u.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
			...
			expect(
				updates.some(
					(u) =>
						...
						u.update?._meta?.[ACRYL_META_NAMESPACE]?.subagents,
				),
			).toBe(true),
		);
		const childUpdates = updates
			.map((u) => u.update?._meta?.[ACRYL_META_NAMESPACE])
			.filter((meta) => meta?.subagents);
	...
	it("does not invent terminal quiescence after cancellation crosses the response boundary", async () => {
		...
		const meta = updates.map((u) => u.update?._meta?.[ACRYL_META_NAMESPACE]).filter(Boolean);
