...
function emitHello(
	...
	socket.emit(
		...
		`${JSON.stringify({
			...
			protocol: { name: "acryl.daemon", version },
			schemaRevision,
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities,
...
describe("DaemonClient", () => {
	...
	it("allows connect retry after the socket emits an error before connecting", async () => {
		...
		expect(firstError.message).toContain("Failed to connect to the ACRYL daemon: initial connect failed.");
		...
		await expect(secondAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Failed to connect to the ACRYL daemon: retry reached socket."),
		});
	});
	...
	it("allows connect retry after the initial connection times out", async () => {
		...
		const timeoutRejection = expect(firstAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Timed out after 5ms connecting to the ACRYL daemon."),
		});
	...
	it("includes command, socket, and log context when a request is made while disconnected", async () => {
		...
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining(
				'Cannot send daemon command "list" because the ACRYL daemon is not connected.',
			),
		});
		await expect(request).rejects.toMatchObject({
	...
	it("notifies listeners when a connected daemon socket closes", async () => {
		...
		expect(closed[0]?.message).toContain("Connection to the ACRYL daemon closed.");
