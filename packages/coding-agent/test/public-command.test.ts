...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("acryl attach <agent>"));
	});
	...
	it("separates ACRYL updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "acryl update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "acryl package update [source]"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "acryl package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("acryl package list"));
	});
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("acryl list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("acryl doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("acryl package install <source>"));
