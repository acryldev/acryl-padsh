...
Prime Intellect is an open superintelligence lab building open-source AGI infrastructure: a platform for RL environments, evaluations, post-training, inference, and globally distributed GPU compute. ACRYL is a fork of Prime Agent, which was built by Prime Intellect, and the `prime` CLI is the default way to interface with every product below.
