...
const UPDATE_RESTART_CONTINUATION_PROMPT =
	"ACRYL restarted after an update. Continue the interrupted task from the saved transcript and restored tool/kernel state. Inspect current state before retrying commands when needed.";

const UPDATE_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		const { noun, pronoun } = pluralizeSessions(count);
		return `ACRYL has ${count} busy ${noun}. After the update installs, it will stop ${pronoun}, restart its background service, and resume interrupted work.`;
	},
	unlistableDetail:
		"Running agents could not be listed. After the update installs, ACRYL will stop resident agents, restart its background service, and resume interrupted work where possible.",
	question: "Continue?",
			...
			const noticeResponse = await client.request(
				{
					...
					message: {
						...
						content: `ACRYL updated to v${VERSION}. This daemon session was restored after the update.`,
						display: true,
