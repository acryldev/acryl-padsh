...
export function formatMissingRipgrepMessage(result: ToolUnavailableResult): string {
	...
	switch (result.reason) {
		...
		case "manual_install_required":
			reason = "ACRYL cannot install this helper automatically in Termux.";
			break;
		...
		case "download_failed": {
			...
			reason = detail
				? `ACRYL could not install it automatically: ${detail}`
				: "ACRYL could not install it automatically.";
			break;
		}
	}

	return [
		"ripgrep (rg) is an optional search helper. Without it, model-run file searches may be slower or fail; ACRYL and subagents remain available.",
		reason,
