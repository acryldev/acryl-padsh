 ...
 * than an error. ACRYL's own package version is far below the Codex CLI's version line, so it must
 ...
 *    explicit, not fetched, so an unlisted model does not exist for ACRYL at all.
