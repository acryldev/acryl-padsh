...
export function getKernelVenvDir(): string {
	const override = process.env.ACRYL_KERNEL_VENV;
	if (override) return path.resolve(expandHome(override));
	return path.join(os.homedir(), ".acryl", "agent", "kernel-venv");
}
...
function getXdgKernelVenvDir(): string {
	...
	return path.join(dataHome, "acryl", "agent", "kernel-venv");
}
...
async function resolveWritableKernelVenvDir(): Promise<string> {
	...
	try {
		...
		if (process.env.ACRYL_KERNEL_VENV) {
		...
		try {
			...
			throw new Error(
				`couldn't create kernel venv directory at ${primary} or ${fallback}; set ACRYL_KERNEL_PYTHON to a python with a current prime-agent-runtime installed. ${errorMessage(fallbackError)}`,
			);
		}
	}
}
...
async function ensureUv(options: EnsureKernelPythonOptions): Promise<string> {
	...
	const shouldInstallUv = process.env.ACRYL_INSTALL_UV === "1" || (!options.onProgress && (await confirmUvInstall()));
	if (!shouldInstallUv) {
		throw new Error(
			`uv is required to set up the Python kernel. Install uv yourself: ${UV_INSTALL_COMMAND}, ` +
				"or set ACRYL_INSTALL_UV=1 to let prime-agent run that installer.",
		);
	}
...
async function confirmUvInstall(): Promise<boolean> {
	if (process.env.ACRYL_INSTALL_UV === "0") return false;
	if (!stdin.isTTY || !stderr.isTTY) return false;

	const rl = createInterface({ input: stdin, output: stderr });
	try {
		const answer = (await rl.question("ACRYL needs uv to set up Python. Install uv from astral.sh now? [Y/n] "))
			.trim()
			.toLowerCase();
...
function formatBootstrapFailure(error: unknown): Error {
	return new Error(
		`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
			...
			"Set ACRYL_KERNEL_PYTHON to a Python with a current prime-agent-runtime and default Python packages installed to skip auto-bootstrap.",
	);
}
...
async function ensureKernelPythonUncached(
	...
	const override = process.env.ACRYL_KERNEL_PYTHON;
	if (override) {
		...
		if (missing.length === 0 && pythonSkills.length > 0) {
			...
			if (missingPythonSkills.length > 0) {
				reportProgress(
					options,
					`Warning: Python skills unavailable in ACRYL_KERNEL_PYTHON and will be disabled: ${missingPythonSkills.join(", ")}`,
				);
			}
		}
		if (missing.length === 0) return python;
		throw new Error(`ACRYL_KERNEL_PYTHON points to a Python missing ${missing.join(" and ")}: ${python}`);
	}
