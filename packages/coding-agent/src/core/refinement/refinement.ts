...
const REFINEMENT_SYSTEM_PROMPT = `You are ACRYL's /refine continual harness subsystem.
...
skills, and subagent specs that lets ACRYL improve reusable behavior
...
- The default editable continual harness store is local to the current ACRYL session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are ACRYL's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this ACRYL session. Global continual harness entries persist across ACRYL sessions.",
