...
/**
 * `daemon ps` discovers every acryl daemon on the machine, not just the
 * one on a single socket. Discovery has two sources merged by socket path:
 *
 *  1. The OS list of listening unix sockets owned by a acryl process
...
/** Parse `ss -lxp` output into the acryl daemons listening on unix sockets. */
	...
	return daemons.map((daemon): ReapAction => {
		...
		if (daemon.status === "unreachable") {
			if (!force || daemon.pid === undefined) {
				return { kind: "skip", daemon, reason: 'unreachable; use "acryl shutdown --force" to stop it' };
			}
...
export async function runShutdownAll(json: boolean, force: boolean): Promise<void> {
	...
	switch (planShutdownConfirmation(daemons.length, json, force, process.stdin.isTTY)) {
		case "json-error":
				...
				JSON.stringify(
					{
						...
						failed: daemons.map(({ socketPath }) => ({
							socketPath,
							reason: 'confirmation required; use "acryl shutdown --force --json"',
						})),
					},
					null,
					2,
			...
			return;
		case "tty-error":
			throw new Error('Shutdown requires confirmation in an interactive terminal. Use "acryl shutdown --force".');
		case "prompt": {
