...
export class DaemonAgentConnection implements AgentConnection {
	...
	async manageHeartbeat(
		...
		if (!this.client.supportsServerCapability("heartbeat_management")) {
			throw new Error("Heartbeat management requires a newer ACRYL daemon.");
		}
		try {
	...
	private formatDaemonSessionClosedError(reason: DaemonSessionClosedReason): string {
		const explanation: Record<DaemonSessionClosedReason, string> = {
			...
			shutdown:
				"The ACRYL daemon shut down while this window was attached. The session transcript remains saved; restart ACRYL and reopen it from Agents View.",
			completed:
			...
			update:
				"The ACRYL daemon restarted for an update, but this window did not restore automatically. The session transcript remains saved; restart ACRYL and reopen it from Agents View.",
		};
		return `${explanation[reason]} ${this.formatDaemonDiagnosticContext()}`;
	}

	private formatDaemonConnectionClosedError(error: Error): string {
		return `Lost connection to the ACRYL daemon. Cause: ${formatErrorSentence(error)} The session transcript remains saved; restart ACRYL or reopen the session from Agents View. ${this.formatDaemonDiagnosticContext()}`;
	}

	private formatUpdateReconnectError(error: unknown): string {
		return `The ACRYL daemon restarted for an update, but this window could not reconnect to its restored session before the recovery timeout expired. Last error: ${formatErrorSentence(error)} The session transcript remains saved; restart ACRYL and reopen it from Agents View. ${this.formatDaemonDiagnosticContext()}`;
	}
	...
	private reconnectAfterUpdate(): Promise<void> {
		...
		void this.emit({
			...
			status: "reconnecting",
			error: "The ACRYL daemon is restarting for an update.",
		});
