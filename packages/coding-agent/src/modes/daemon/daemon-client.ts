...
export class DaemonSocketClosedError extends Error {
	constructor(
		...
		super(
			`Connection to the ACRYL daemon closed.${reasonDetails}${causeDetails} ${daemonEndpointDetails(socketPath)}`,
		);
...
export class DaemonCapabilityUnavailableError extends Error {
	constructor(
		...
		super(
			capability
				? `The running ACRYL daemon does not support ${capability}.`
				: `The running ACRYL daemon does not support ${command}.`,
		);
...
export class DaemonClient {
	...
	async waitForHello(timeoutMs = 3000): Promise<DaemonHello> {
		...
		if (!this.socket || this.socket.destroyed) {
			throw new Error(
				`Cannot wait for the ACRYL daemon handshake because the daemon is not connected. ${daemonEndpointDetails(this.socketPath)}`,
			);
		}
		return new Promise<DaemonHello>((resolve, reject) => {
			const waiter = {
				...
				timeout: setTimeout(() => {
					...
					reject(
						new Error(
							`Timed out after ${timeoutMs}ms waiting for the ACRYL daemon handshake. ${daemonEndpointDetails(this.socketPath)}`,
						),
					);
				}, timeoutMs),
			};
	...
	async connect(timeoutMs = 3000): Promise<void> {
		if (this.socket) {
			throw new Error(`ACRYL daemon client is already connected. ${daemonEndpointDetails(this.socketPath)}`);
		}
		...
		await new Promise<void>((resolve, reject) => {
			const timeout = setTimeout(() => {
				...
				reject(
					new Error(
						`Timed out after ${timeoutMs}ms connecting to the ACRYL daemon. ${daemonEndpointDetails(this.socketPath)}`,
					),
				);
			}, timeoutMs);
			const cleanup = () => {
			...
			const onError = (error: Error) => {
				...
				reject(
					new Error(
						`Failed to connect to the ACRYL daemon: ${error.message}. ${daemonEndpointDetails(this.socketPath)}`,
					),
				);
			};
	...
	async request(
		...
		if (!this.socket || this.socket.destroyed) {
			throw new Error(
				`Cannot send daemon command "${command.type}" because the ACRYL daemon is not connected. ${daemonEndpointDetails(this.socketPath)}`,
			);
		}
	...
	private armPendingRequestTimeout(id: string, pending: PendingDaemonRequest): void {
		pending.timeout = setTimeout(() => {
			...
			pending.reject(
				new Error(
					`Timed out after ${pending.timeoutMs}ms waiting for the ACRYL daemon response to "${pending.commandType}". ${daemonEndpointDetails(this.socketPath)}`,
				),
			);
		}, pending.timeoutMs);
	}
	...
	close(): void {
		...
		this.rejectAll(
			new Error(
				`ACRYL daemon client closed before the operation completed. ${daemonEndpointDetails(this.socketPath)}`,
			),
		);
		this.socket?.end();
		this.socket?.destroy();
