...
ACRYL supports subscription-based providers via OAuth and API key providers via environment variables or the auth file. Its built-in model catalog is updated with each ACRYL release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.acryl/agent/auth.json` and auto-refresh when expired.
...
Store credentials in `~/.acryl/agent/auth.json`:
...
```bash
acryl --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
acryl --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
acryl --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal ACRYL usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
acryl --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

ACRYL automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
