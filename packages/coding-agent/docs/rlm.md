...
Each `bash()` call is its own process, while Python state, `os.chdir(...)`, and `os.environ[...]` changes persist in the kernel and apply to later `bash()` calls. ACRYL extensions may intentionally add custom tools, but the built-in RLM design does not require a separate model tool for every capability.
...
ACRYL supports the Agent Skills markdown format and extends it with Python-backed skills. Both use `SKILL.md` for discovery, routing, and instructions. A Python-backed skill also contains a Python package that ACRYL installs into the kernel environment and exposes by import name.
