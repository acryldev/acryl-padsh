...
ACP mode makes ACRYL an [Agent Client Protocol](https://agentclientprotocol.com) agent, speaking JSON-RPC 2.0 over newline-delimited JSON on stdin/stdout. Any ACP client — an editor like Zed or VS Code, or an evaluation harness — can drive it without knowing anything ACRYL-specific.

```bash
acryl --mode acp
```

Use ACP mode when something external needs to *drive* a session interactively: prompt, watch tool calls stream, cancel a turn. For batch runs where you want every event dumped and an exit code, [JSON event stream mode](json.md) is a better fit. [RPC mode](rpc.md) remains available and exposes ACRYL's own richer command surface.
...
One session per connection is a deliberate limit: ACRYL's underlying session is fixed at process startup, so a second concurrent session would silently share its conversation, working directory, and model. A second `session/new` is refused rather than pretending to isolate. Start another process for a second session.
...
ACRYL accepts standard stdio and HTTP servers in `session/new.mcpServers`. The servers are
...
`auth.json`, start or refresh ACRYL OAuth, or modify persistent MCP settings. Stdio servers
...
ACP stdio is a trusted-code boundary, not a sandbox. The requested command runs as the ACRYL
user and can access any files that user can access, including credential stores. Only accept stdio
servers from trusted ACP clients or run ACRYL inside an appropriate sandbox.

## Streamed updates

Session activity arrives as `session/update` notifications:

| ACRYL activity | ACP update |
|---|---|
...
The Python REPL is ACRYL's model-facing tool, so a cell is a `tool_call` of kind `execute` whose `rawInput` carries the cell source.

## ACRYL extensions

ACRYL has capabilities ACP has no field for: subagents, autonomous quality gates, goals, heartbeats, continual-harness refinement, compaction, and rich kernel output. These travel in a reverse-domain `_meta` envelope:
...
A standard ACP client ignores `_meta` entirely and still works. A ACRYL-aware client, or a harness that cares about subagent trees and gate attempts, reads it. Nothing non-standard is ever added to an ACP object root, which the protocol reserves for future fields.
