...
```
~/.acryl/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.acryl/agent/sessions/`.

ACRYL also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, ACRYL uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from ACRYL AI)
...
### Extended Message Types (from ACRYL Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by ACRYL (legacy field name)
