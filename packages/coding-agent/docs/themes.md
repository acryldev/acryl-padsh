> ACRYL can create themes. Ask it to build one for your setup.
...
ACRYL loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.acryl/agent/themes/*.json`
- Project: `.acryl/agent/themes/*.json`
...
On first run, ACRYL detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.acryl/agent/themes
vim ~/.acryl/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, ACRYL reloads it automatically for immediate visual feedback.
...
ACRYL uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, ACRYL falls back to the nearest approximation.
