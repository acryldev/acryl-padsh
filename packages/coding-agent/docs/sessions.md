...
ACRYL saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.acryl/agent/sessions/`. Each session is a JSONL file with a tree structure.

```bash
acryl --continue          # Continue the most recent session
acryl --resume [path|id]  # Browse past sessions or resume one directly
acryl --no-session        # Ephemeral mode; do not save
acryl --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `acryl --resume` opens the same picker at startup, and `acryl --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `acryl --resume -- "continue this work"`.
...
When available, ACRYL uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `acryl --resume`.
...
When `/tree` switches away from one branch to another, ACRYL can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
