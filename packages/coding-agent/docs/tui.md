> ACRYL can create TUI components. Ask it to build one for your use case.
...
ACRYL shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while ACRYL is streaming a response.
...
// Restore ACRYL's default spinner
ctx.ui.setWorkingIndicator();
```
