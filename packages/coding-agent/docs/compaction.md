...
LLMs have limited context windows. When conversations grow too long, ACRYL uses compaction to summarize older content while preserving recent work. This page covers both auto-compaction and branch summarization.
...
ACRYL has two summarization mechanisms:
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.acryl/agent/settings.json` or `<project-dir>/.acryl/agent/settings.json`). This leaves room for the LLM's response.
...
1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.acryl/agent/settings.json` or `<project-dir>/.acryl/agent/settings.json`) is reached
...
On repeated compactions, the summarized span starts at the previous compaction's kept boundary (`firstKeptEntryId`), not at the compaction entry itself, falling back to the entry after the previous compaction if that kept entry cannot be found in the path. This preserves messages that survived the earlier compaction by including them in the next summarization pass as well. ACRYL also recalculates `tokensBefore` from the rebuilt session context before writing the new `CompactionEntry`, so the token count reflects the actual pre-compaction context being replaced.
...
For split turns, ACRYL generates two summaries and merges them:
...
When you use `/tree` to navigate to a different branch, ACRYL offers to summarize the work you're leaving. This injects context from the left branch into the new branch.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, ACRYL extracts file operations from:
...
Configure compaction in `~/.acryl/agent/settings.json` or `<project-dir>/.acryl/agent/settings.json`:
